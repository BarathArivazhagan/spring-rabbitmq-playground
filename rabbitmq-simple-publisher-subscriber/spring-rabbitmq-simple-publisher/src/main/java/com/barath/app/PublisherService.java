...
public class PublisherService {
	...
	public String publishAndReceiveMessage(String message){
		...
		Message responseMessage=rabbitTemplate.sendAndReceive(syncQueueName,MessageBuilder.withBody(message.getBytes()).build());
		Optional.ofNullable(responseMessage).ifPresent( response -> logger.info("Response message from Rabbit {}",response.getBody()));
